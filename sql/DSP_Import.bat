...
"c:\program files\mysql\mysql server 5.7\bin\mysqladmin"-h localhost -u root -pFj@50079 DROP dspdb

ECHO Creating Database dspdb
"c:\program files\mysql\mysql server 5.7\bin\mysqladmin"-h localhost -u root -pFj@50079 CREATE dspdb

ECHO Loading dspdb tables into the database
cd c:\darkstar\sql
FOR %%X IN (*.sql) DO ECHO Importing %%X & "c:\program files\mysql\mysql server 5.7\bin\mysql" dspdb -h localhost -u root -pFj@50079 < %%X
